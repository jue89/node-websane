...
const NON_LEAP_DAYS_PER_MONTH: [u8; 12] = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
const LEAP_DAYS_PER_MONTH: [u8; 12] = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
...
pub fn system_time_to_string(time: SystemTime) -> String {
    ...
    month += 1;
    days += 1;
    return format!("{year:04}-{month:02}-{days:02}T{hours:02}:{minutes:02}:{seconds:02}Z");
}
